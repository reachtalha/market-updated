...
export default function OrderSummaryCheckout({ isConfirmButtonLoading = false } : OrderSummaryCheckoutProps) {
  const { cart, isCartLoading } = useCartStore((state: any) => state);

  return isCartLoading ? <Skeleton className="bg-gray-200 h-[300px]" /> : (
