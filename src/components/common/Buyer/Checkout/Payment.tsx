...
const Payment = ({ children } : { children: ReactNode}) => {
  ...
  const { data, isLoading } = useSwr(() => cart?.summary?.total != null ? 'payment_intent_creation' : null, () => fetchCreatePaymentIntent(cart?.summary?.total), {
  ...
  return (isLoading || cart?.summary?.total == null) ? <Skeleton className="h-[100px] w-full bg-gray-200" /> : (
