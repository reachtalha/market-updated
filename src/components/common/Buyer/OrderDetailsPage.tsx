...
const OrderDetailsPage = ({ dictionary, orderId }: Props) => {
  const router = useRouter();

  const { data, isLoading, error } = useSwr('mobile-orders', () => getOrder(orderId));
  ...
  return (
    <div className=" w-[90%] m-auto mt-20">
      ...
      <OrderDetail dictionary={dictionary} order={data} />
    </div>
  );
};
