    ...
    <Sheet open={isOpen} onOpenChange={() => setIsOpen(!isOpen)}>
      <SheetTrigger asChild>
        <Button className="text-black hover:bg-transparent p-0" variant="ghost">
          <HamburgerMenuIcon height={30} width={30} className={twMerge(`text-white`, (isScrollChanged || isColoredRoute(pathname)) && 'text-black' )} />
        </Button>
      </SheetTrigger>
          ...
          <SheetDescription>
            <div className="flex flex-col mt-8">
              <SheetClose className="text-left  border-b border-gray-300">
                <NavLink onClick={closeSheet} className="inline-block py-2 w-full" href="/for-you" title="For You" />
              </SheetClose>
              <SheetClose asChild className="text-left border-b border-gray-300">
                <NavLink onClick={closeSheet} className="inline-block py-2 w-full" href="/products" title="All Products" />
              </SheetClose>
              <SheetClose className="text-left  border-b border-gray-300">
                <NavLink onClick={closeSheet} className="inline-block py-2 w-full" href="/market" title="Market" />
              </SheetClose>
              <SheetClose className="text-left  border-b border-gray-300">
                <NavLink onClick={closeSheet} className="inline-block py-2 w-full" href="/experts" title="Experts" />
              </SheetClose>
              <SheetClose className="text-left w-full border-b border-gray-300">
                <NavLink onClick={closeSheet} className="inline-block py-2" href="/account" title="Account" />
              </SheetClose>
            </div>
          </SheetDescription>
        </SheetHeader>
